...
function UserRanking() {
  ...
  .limit(1).get().then((snapshot)=> {
    snapshot.forEach(doc=> {
        ...
        str += '<div class="leader-rank"><div class="Rainking-number" style="color:#f68b1f;">'+addCommas(doc.data().TopYear)+'</div><div style="font-size:10px;color:#fff;">อันดับ</div></div>';
        str += '<div class="leader-rank"><div class="Rainking-number" style="color:#f68b1f;">'+addCommas(doc.data().TotalPoint.toFixed(0))+'</div><div style="font-size:10px;color:#fff;">คะแนน</div></div>';
        str += '</div>';        
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      if(doc.data().TotalPoint>0) {
        ...
        str += '<div class="leader-rank"><div class="Rainking-number">'+addCommas(doc.data().TotalPoint.toFixed(0))+'</div><div style="font-size:10px;">คะแนน</div></div>';
        str += '</div>';        
      }
    });
